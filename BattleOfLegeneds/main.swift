...
struct Sword: Weapon{
    var name: String = "Khamozaki"
    var description: String  = "Killed you"
    var effectPoints: Int = 10
    
    func attack<P>(_ player: P) where P : Player {
        if player.health > effectPoints {
            player.health -= effectPoints
            print("\(player.name) took \(effectPoints) damage. Current health: \(player.health)")
        }else if player.health < effectPoints{
            print("GG")
        }
    }
}
...
struct Knife: Weapon{
    ...
    struct HealthPotion: Potion{
        var name: String = "Healing Elixir"
        var description: String = "Restores health"
        var effectPoints: Int = 20
        
        func splash<P>(_ player: P) where P : Player {
            player.health += effectPoints
                   print("\(player.name) healed for \(effectPoints) points. Current health: \(player.health)")
               }
        }
    ...
    struct PoisonPotion: Potion{
        var name: String = "Deadly Poison"
        var description: String = "Inflicts poison"
        var effectPoints: Int = 20
        func splash<P>(_ player: P) where P : Player {
            if player.health > effectPoints {
              player.health -= effectPoints
              print("\(player.name) took \(effectPoints) poison damage. Current health: \(player.health)")
                    } else{
                print("GG")
            }
        }
...
class Warrior: Player{
    ...
    init(name: String, chest: Chest, health: Int) {
        self.name = name
        self.chest = chest
        self.health = health
    }
    ...
    func splash<P>(_ player: P) where P : Player {
        for item in chest.items where item is PoisonPotion{
            if let poisonPotion = item as? PoisonPotion {
                poisonPotion.splash(self)
                return
            }
        }
    }
...
let warrior = Warrior(name: "Aragon" , chest: Chest.init(items: []) , health: 100 )
let wizard = Wizard(name: "Gandalf", chest: Chest.init(items: []) , health: 100 )
